'''
Title: multi-resolution-texture-synthesis
Author: anopara
Date: June 6, 2020
Code version: commit 92144e96ea10ceb517416b9bf3c8645adefd0c73
Availability: https://github.com/anopara/multi-resolution-texture-synthesis 
'''
...
def multiResolution_textureSynthesis(parms, userExample = None):
    ...
    if parms["pyramidType"] == "gaussian":
        exampleMap = gaussianPyramid(loadExampleMap(parms["exampleMapPath"],(parms['outputSize'][0],parms['outputSize'][1])), levels = parms["pyramidLevels"])
        canvas = gaussianPyramid(initCanvas(parms["outputSize"]), levels = exampleMap.levels)
    elif parms["pyramidType"] == "laplacian":
        exampleMap = laplacianPyramid(loadExampleMap(parms["exampleMapPath"],(parms['outputSize'][0],parms['outputSize'][1])), levels = parms["pyramidLevels"])
    ...
    if userExample is not None:
        ...
        userExampleImg = loadExampleMap(userExample["userExamplePath"],(parms['outputSize'][0],parms['outputSize'][1]))
...
def saveImg(canvas, pyramidType, pyramidLevel, savePath, index):
    ...
    img = img.resize((256, 256), resample=0, box=None)
...
def loadExampleMap(exampleMapPath,size):
    exampleMap = io.imread(exampleMapPath) #returns an MxNx3 array
    # exampleMap = skimage.transform.resize(exampleMap,size)
...
def tsvq(image_path, output_path, output_size=[256, 256],
        child_kernel_size=5,parent_kernel_size=3,
        pyramid_levels=4, pyramid_type='gaussian'):

    parms = {
        "exampleMapPath": image_path,
        "outputSize": output_size,
        "child_kernel_size": child_kernel_size,
        "parent_kernel_size": parent_kernel_size,
        "saveImgsPath": output_path,
        "saveGifPath": "./out.gif",
