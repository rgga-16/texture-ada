...
class Pyramid2D_2(nn.Module):
    def __init__(self, ch_in=3, ch_step=8, n_samples=6):
        super(Pyramid2D_2, self).__init__()
    ...
    def forward(self, z):
        ...
        y = self.cb4_2(y) # y=(24x64x64) => (32x64x64)
        y = self.up4(y) # y=(32x128x128)

        y = self.cb5_2(y) # y=(32x128x128) => (40x128x128)
        y = self.up5(y) # y=(40x128x128) => (40x256x256)

        y = self.cb6_2(y) # y=(40x256x256) => (48x256x256)
        y = self.last_conv(y) # y=(48x256x256) => (3x256x256)
        return y
